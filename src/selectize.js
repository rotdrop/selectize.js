...
$.extend(Selectize.prototype, {
    ...
    noArrowClass      = settings.showArrow ? '' : ' no-arrow';
	  ...
	  $control_input    = $('<input type="text" autocomplete="new-password" autofill="no" />').addClass((settings.inputClass ? settings.inputClass + '-element ' : '')+ 'selectize-input-element').appendTo($control).attr('tabindex', $input.is(':disabled') ? '-1' : self.tabIndex);
		...
		var templates = {
			...
			'option': function(data, escape) {
				...
				if (data[field_tooltip]) {
					return '<div' + styles + ' class="option' + classes + '" title="' + data[field_tooltip] + '">' + escape(data[field_label]) + '</div>';
				} else {
			...
			'item': function(data, escape) {
				if (data[field_tooltip]) {
					return '<div class="item" title="' + data[field_tooltip] + '">' + escape(data[field_label]) + '</div>';
				} else  {
		...
		for (i = 0; i < n; i++) {
			...
			for (j = 0, k = optgroups && optgroups.length; j < k; j++) {
				...
				if (!self.optgroups.hasOwnProperty(optgroup) && typeof self.settings.optionGroupRegister === 'function') {
					var regGroup = self.settings.optionGroupRegister.apply(self, [optgroup]);
					if (regGroup) {
	...
	addOption: function(data) {
		...
		value = self.registerOption(data);
		if (value) {
